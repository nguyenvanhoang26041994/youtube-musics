...
const SongSmallCard = ({ className, name, singersName, img, onClick, ...otherProps }) => {
  ...
  return (
    ...
    >
      ...
      <div className={cn('flex flex-col flex-1 ml-2 justify-center')}>
        <div className="ui-song-small-card__name transition-fast text-sm text-indigo-500 w-full truncate">{name}</div>
        <div className="ui-song-small-card__singersName text-xs text-gray-500 w-full truncate">{singersName}</div>
      </div>
